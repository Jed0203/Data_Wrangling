...
```{r echo = FALSE}
download("https://github.com/fivethirtyeight/guns-data/raw/master/full_data.csv", dest = "gun_death.csv", mode = "wb")
gun_death <- read_csv("gun_death.csv")
```
...
race_chart <- ggplot(race_death)+
  geom_bar(aes(x=race, fill=race))
```


```{r message=FALSE, warning=FALSE}
gender_death <- gun_death %>% 
  group_by(sex)

gender_chart <- ggplot(gender_death)+
  geom_bar(aes(x=sex, fill=sex))
```


```{r message=FALSE, warning=FALSE}
intent_death <- gun_death %>% 
  group_by(intent)

intent_chart <- ggplot(intent_death)+
...
```{r message=FALSE, warning=FALSE}
female_education_dat <- gun_death %>% 
